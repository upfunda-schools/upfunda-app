...
const Map<String, Map<String, int>> avatarPartPrices = {
  ...
  'bgColor': {'#F44336': 10, '#4CAF50': 10, '#2196F3': 10, '#9C27B0': 10, '#FFFFFF': 0, '#FCED70': 0},
...
class _AvatarEditorScreenState extends ConsumerState<AvatarEditorScreen> {
  ...
  void _initializeUnlockedParts() {
    final profile = ref.read(userProvider).profile;
    final name = profile?.name ?? 'default';
    final trimmedName = name.trim();
    final lowerName = trimmedName.toLowerCase();
    
    // 1. Load from profile map (database) - Ultra-Compatible lookup
    final purchased = profile?.rawPurchasedMap?[name] ?? 
                      profile?.rawPurchasedMap?[trimmedName] ?? 
                      profile?.rawPurchasedMap?[lowerName];
                      
    if (purchased != null && purchased is List) {
      for (var item in purchased) {
        if (item is Map && item.containsKey('part') && item.containsKey('value')) {
    ...
    // 3. Add currently equipped (to be safe)
    _unlockedParts.add('faceColor:${_currentConfig.faceColor}');
    _unlockedParts.add('bgColor:${_currentConfig.bgColor}');
    _unlockedParts.add('hairStyle:${_currentConfig.hairStyle}');
    _unlockedParts.add('eyeStyle:${_currentConfig.eyeStyle}');
    _unlockedParts.add('mouthStyle:${_currentConfig.mouthStyle}');
    _unlockedParts.add('shirtStyle:${_currentConfig.shirtStyle}');
    _unlockedParts.add('hatStyle:${_currentConfig.hatStyle}');
    _unlockedParts.add('eyeBrowStyle:${_currentConfig.eyeBrowStyle}');
    _unlockedParts.add('glassesStyle:${_currentConfig.glassesStyle}');
    _unlockedParts.add('earSize:${_currentConfig.earSize}');
    _unlockedParts.add('noseStyle:${_currentConfig.noseStyle}');
  }
  ...
  Widget _buildOptionsGrid() {
    switch (_activeCategory) {
      ...
      case 'bgColor':
        return _buildColorOptions(['#FCED70', '#F44336', '#4CAF50', '#2196F3', '#9C27B0', '#FFFFFF'], 'bgColor');
      case 'hairStyle':
    ...
    if (svg != null) {
      return Padding(
        ...
        child: SvgPicture.string(
          svg,
          fit: BoxFit.contain,
          colorFilter: field == 'eyeStyle' || field == 'mouthStyle' || field == 'noseStyle' || field == 'eyeBrowStyle' || field == 'glassesStyle'
  ...
  String _getShirtSvgForStyle(String style) {
    final color = _currentConfig.shirtColor ?? '#2196F3';
    switch (style) {
        ...
        content: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: ClipOval(
                child: AvatarDisplay(config: _currentConfig),
              ),
            ),
            const SizedBox(height: 32),
            Row(
  ...
  void _handleRemove() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Are you sure you want to remove your avatar?', style: TextStyle(color: Colors.white70)),
        actions: [
